...
class NordlowGC : GC
{
    ...
    static void initialize(ref GC gc)
    {
        ...
        if (config.gc != "nordlow")
            return;

        auto p = cstdlib.malloc(__traits(classInstanceSize, NordlowGC));
        if (!p)
            onOutOfMemoryError();

        auto init = typeid(NordlowGC).initializer();
        assert(init.length == __traits(classInstanceSize, NordlowGC));
        auto instance = cast(NordlowGC) memcpy(p, init.ptr, init.length);
        instance.__ctor();
    ...
    static void finalize(ref GC gc)
    {
        ...
        auto instance = cast(NordlowGC) gc;
        instance.Dtor();
